...
test('multisig-vlei-issuance', async function run() {
    ...
    try {
        ...
        // Not sent because they already created the AID
    ...
    if (oobiGEDAbyGAR1.oobis.length == 0 || oobiGEDAbyGAR2.oobis.length == 0) {
        ...
        const opList1 = await addEndRoleMultisig(
            clientGAR1,
            await clientGAR1.identifiers().get('GEDA'),
            timestamp
        );
        const opList2 = await addEndRoleMultisig(
            clientGAR2,
            await clientGAR2.identifiers().get('GEDA'),
            timestamp
        );
    ...
    try {
        ...
        await waitAndMarkNotification(clientGAR2, '/multisig/ixn');
    ...
    if (
        ...
        const opList1 = await addEndRoleMultisig(
            clientQAR1,
            await clientQAR1.identifiers().get('QVI'),
            timestamp
        );
        const opList2 = await addEndRoleMultisig(
            clientQAR2,
            await clientQAR2.identifiers().get('QVI'),
            timestamp
        );
        const opList3 = await addEndRoleMultisig(
            clientQAR3,
            await clientQAR3.identifiers().get('QVI'),
            timestamp
        );
        ...
        await waitAndMarkNotification(clientQAR3, '/multisig/rpy');
    ...
    if (gedaRegistrybyGAR1.length == 0 && gedaRegistrybyGAR2.length == 0) {
        ...
        gedaRegistrybyGAR1 = await retry(async () => {
            const regs = await clientGAR1.registries().list(aidGEDA.name);
            assert(regs.length > 0, 'Registry not found yet');
            return regs;
        });

        gedaRegistrybyGAR2 = await retry(async () => {
            const regs = await clientGAR2.registries().list(aidGEDA.name);
            assert(regs.length > 0, 'Registry not found yet');
            return regs;
        });
    }
    ...
    if (
        ...
        const opList1 = await addEndRoleMultisig(
            clientLAR1,
            await clientLAR1.identifiers().get('LE'),
            timestamp
        );
        const opList2 = await addEndRoleMultisig(
            clientLAR2,
            await clientLAR2.identifiers().get('LE'),
            timestamp
        );
        const opList3 = await addEndRoleMultisig(
            clientLAR3,
            await clientLAR3.identifiers().get('LE'),
            timestamp
        );
    ...
    if (
        ...
        qviRegistrybyQAR1 = await retry(async () => {
            const regs = await clientQAR1.registries().list(aidQVI.name);
            assert(regs.length > 0, 'Registry not found yet');
            return regs;
        });

        qviRegistrybyQAR2 = await retry(async () => {
            const regs = await clientQAR2.registries().list(aidQVI.name);
            assert(regs.length > 0, 'Registry not found yet');
            return regs;
        });
        qviRegistrybyQAR3 = await retry(async () => {
            const regs = await clientQAR3.registries().list(aidQVI.name);
            assert(regs.length > 0, 'Registry not found yet');
            return regs;
        });
    }
    ...
    if (
        ...
        leRegistrybyLAR1 = await retry(async () => {
            const regs = await clientLAR1.registries().list(aidLE.name);
            assert(regs.length > 0, 'Registry not found yet');
            return regs;
        });

        leRegistrybyLAR2 = await retry(async () => {
            const regs = await clientLAR2.registries().list(aidLE.name);
            assert(regs.length > 0, 'Registry not found yet');
            return regs;
        });
        leRegistrybyLAR3 = await retry(async () => {
            const regs = await clientLAR3.registries().list(aidLE.name);
            assert(regs.length > 0, 'Registry not found yet');
            return regs;
        });
    }
