...
let registry: { regk: string; name: string };
let holderRegistry: { regk: string; name: string };
...
test('Resolve schema oobis', async () => {
    await Promise.all([
...
test('Create registry', async () => {
    ...
    registry = await retry(async () => {
        const registries = await issuerClient.registries().list(issuerAid.name);
        assert.equal(registries.length, 1);
        assert.equal(registries[0].name, updatedRegistryName);
        return registries[0];
    });
});

test('issuer can get schemas', async () => {
    const issuerQviSchema = await issuerClient.schemas().get(QVI_SCHEMA_SAID);
...
test('create QVI credential', async () => {
    ...
    qviCredentialId = issResult.acdc.sad.d as string;
});
...
test('issuer list credentials with filter', async () => {
    assertLength(
        await issuerClient
            .credentials()
...
test('holder can get the credential status before or without holding', async () => {
...
test('issuer IPEX grant response', async () => {
...
test('holder has credential', async () => {
    const holderCredential = await retry(async () => {
        const result = await holderClient.credentials().get(qviCredentialId);
...
test('holder IPEX apply receive and offer', async () => {
    ...
    const filter: Record<string, string> = { '-s': apply.exn.a.s };
...
test('holder create registry for LE credential', async () => {
    ...
    holderRegistry = await retry(async () => {
...
test('holder create LE (chained) credential', async () => {
    const qviCredential = await holderClient.credentials().get(qviCredentialId);

    const result = await holderClient.credentials().issue(holderAid.name, {
        a: {
    ...
    leCredentialId = result.acdc.sad.d;
});
...
test('LE credential IPEX grant response', async () => {
    const notifications = await waitForNotifications(
        holderClient,
        '/exn/ipex/admit'
    );
...
test('Issuer revoke QVI credential', async () => {
...
test('Holder deletes LE credential', async () => {
